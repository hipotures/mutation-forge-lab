...
def test_provider_transport_artifact_path_is_part_of_frozen_request() -> None:
    ...
    request = build_provider_request(
        ...
        system_prompt="system",
        output_schema={"type": "object"},
        artifact_dir="/workspace/artifacts/slots/slot-00",
        artifact_prefix="slot-00",
    )

    assert request.request["artifact_dir"] == (
        "/workspace/artifacts/slots/slot-00"
    )
    assert request.request["artifact_prefix"] == "slot-00"
    assert b'"artifact_prefix":"slot-00"' in request.encoded_bytes
