...
from .artifacts import GenerationArtifacts, replay_generation
...
from .isolation import IsolatedCapsule
from .manifest import THREAD_ENVIRONMENT, load_manifest, read_cpu_topology, sha256
...
def appserver_doctor(config_path: str | Path) -> dict[str, Any]:
    ...
    try:
        ...
        checks["model"] = {"ok": False, "error": type(error).__name__}
        checks["profiles"] = {"ok": False, "error": type(error).__name__}
        checks["protocol"] = {"ok": False, "error": type(error).__name__}
    finally:
    ...
    return canonical_result(
        {
            ...
            "status": "completed" if offline_ok and auth_ok else "failed",
...
def _verify_freeze_inputs(config: Stage3GenerationConfig) -> dict[str, Any]:
    ...
    if tag_commit is not None and not tag_ok:
        raise RuntimeError("existing preregistration tag does not point at the current commit")
    ...
    if [p.stem for p in slots] != list(SLOTS):
    ...
    return {
        "project": project,
        "heg": heg,
        "tag": {"name": config.preregistration_tag, "commit": tag_commit},
        "hashes": hashes,
...
def freeze(config_path: str | Path) -> dict[str, Any]:
    config = load_stage3_config(config_path)
    audit = appserver_doctor(config_path)
    if audit["offline_validation"] != "passed":
        raise RuntimeError("offline app-server protocol/profile audit failed; refusing freeze")
    ...
    payload = {
        ...
        "inference": False,
        "auth_ready": bool(audit["live_generation_ready"]),
        "doctor": audit,
...
def generate(
    ...
    *,
    ...
    run_id = f"stage3-generation-{config.stable_hash()[:12]}"
    artifacts = GenerationArtifacts(config.run_root, run_id)
    artifacts.start(
        {
    ...
    if not auth.get("authenticated", False):
        ...
        failure = {
            ...
            "failure": {
                "code": "private_capsule_auth_unavailable",
                "message": (
    ...
    for slot in SLOTS:
        brief_value = json.loads((config.slot_briefs_dir / f"{slot}.json").read_text())
        if (
            not isinstance(brief_value, Mapping)
            or brief_value.get("slot_id") != slot
            or not isinstance(brief_value.get("brief"), str)
        ):
            raise RuntimeError(f"invalid frozen slot brief {slot}")
        slot_requests[slot] = {
            "slot": slot,
            "model": config.model.name,
            "effort": config.model.effort,
            ...
            "prompt": prompt_bundle.render_slot_request(slot, cast(str, brief_value["brief"])),
...
def _run_evaluation_shard(
    payload: tuple[str, list[Mapping[str, Any]], Mapping[str, str], int],
) -> tuple[list[dict[str, Any]], dict[str, Any]]:
    """Run one immutable shard on one physical CPU."""
    config_path, episodes, policies, cpu_id = payload
...
def _run_evaluation_pass(
    ...
    payloads = [
        (str(config.source_path), shard, policies, worker_cores[index].cpu_id)
...
def evaluate(
    ...
    *,
    ...
    if generation_summary.get("status") != "completed":
        return canonical_result(
            {
                "status": "infrastructure_failure",
                "decision": "INCONCLUSIVE_INFRASTRUCTURE_FAILURE",
    ...
    selected = (
        ...
        else cast(list[Mapping[str, Any]], manifest["episodes"])
    )
    ...
    frozen_doctor = frozen.get("doctor")
    protocol_safety = (
...
def verify_replay(primary: str | Path, replay: str | Path) -> dict[str, Any]:
    def load(value: str | Path) -> dict[str, Any]:
        path = Path(value)
        if path.is_dir():
            return replay_generation(path)
        return cast(dict[str, Any], json.loads(path.read_text(encoding="utf-8")))

    left, right = load(primary), load(replay)
    volatile = {"created_at", "finished_at", "timing", "path", "run_path"}
    ...
    left_hash = hashlib.sha256(
        json.dumps(projection(left), sort_keys=True, separators=(",", ":")).encode()
    ).hexdigest()
    right_hash = hashlib.sha256(
        json.dumps(projection(right), sort_keys=True, separators=(",", ":")).encode()
    ).hexdigest()
    equal = left_hash == right_hash
    declared_mismatches = sorted(
        key
        for key in set(left) | set(right)
        if key.endswith("_sha256") and left.get(key) != right.get(key)
    )
    decision_match = left.get("decision") == right.get("decision")
    equal = equal and not declared_mismatches and decision_match
    return canonical_result(
        {
            "status": "completed" if equal else "failed",
            "decision": "exact" if equal else "mismatch",
